...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8):
    ...
    if _results_exist() and not debug:
        print('FPS: {}'.format(-1), flush=True)
        return

    print('Tracker: {} {} {} ,  Sequence: {}'.format(tracker.name, tracker.parameter_name, tracker.run_id, seq.name), flush=True)
    ...
    print('FPS: {}'.format(num_frames / exec_time), flush=True)
...
def run_dataset(dataset, trackers, debug=False, threads=0, num_gpus=8):
    ...
    print('Evaluating {:4d} trackers on {:5d} sequences'.format(len(trackers), len(dataset)), flush=True)
    ...
    print('Done, total time: {}'.format(str(timedelta(seconds=(time.time() - dataset_start_time)))), flush=True)
