...
class Sequence:
    ...
    def init_info(self, frame_num=0):
        ...
        # return info
        frame_num = 0
        info = self.frame_info(frame_num)
        if info['init_bbox'][-2] != 0 and info['init_bbox'][-1] != 0 :
            return info, frame_num
        else : 
            while (self.ground_truth_rect[frame_num][-2] == 0 and self.ground_truth_rect[frame_num][-1] == 0):
                frame_num += 1
            return {'init_bbox':self.ground_truth_rect[frame_num]}, frame_num
