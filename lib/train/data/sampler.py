...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pos_prob=0.5, dynamic_nums=7):
        """
        args:
        ...
        self.dynamic_nums = dynamic_nums
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                elif self.frame_sample_mode == "mamba":
                    template_frame_ids, search_frame_ids = self.get_frame_ids_mamba(visible, seq_info_dict["valid"])
                else:
    ...
    def get_frame_ids_mamba(self, visible, valid):
        ...
        while None in template_frame_ids_extra or len(template_frame_ids_extra) == 0:
            ...
            if len(remaining_ids) < self.dynamic_nums:
                template_frame_ids_extra += [None]
                continue

            # Sample the extra template frame IDs
            template_frame_ids_extra = random.sample(remaining_ids, self.dynamic_nums)
        ...
        # def get_frame_ids_mamba(self, visible, valid):
    #     # get template and search ids in a 'mamba' manner
    #     template_frame_ids_extra = []
    #     while None in template_frame_ids_extra or len(template_frame_ids_extra) == 0:
    #         template_frame_ids_extra = []
    #         # first randomly sample two frames from a video
    #         template_frame_id0 = self._sample_visible_ids(visible, num_ids=1)  # the initial template id
    #         search_frame_ids = self._sample_visible_ids(visible, num_ids=1)  # the search region id
    #         # get the dynamic template id
    #         if isinstance(self.max_gap, int):
    #             # 注意这里是相当于写死了！！！
    #             self.max_gap = [len(visible)]
    #         for max_gap in self.max_gap:
    #             if template_frame_id0[0] >= search_frame_ids[0]:
    #                 min_id, max_id = search_frame_ids[0], search_frame_ids[0] + max_gap
    #             else:
    #                 min_id, max_id = search_frame_ids[0] - max_gap, search_frame_ids[0]
    #             """we require the frame to be valid but not necessary visible"""
    #             f_id = self._sample_visible_ids(valid, num_ids=6, min_id=min_id, max_id=max_id)
    #             if f_id is None:
    #                 template_frame_ids_extra += [None]
    #             else:
    #                 template_frame_ids_extra += f_id
