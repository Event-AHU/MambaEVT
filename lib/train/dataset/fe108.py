...
class Fe108(BaseVideoDataset):
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        # frame_list = [self._get_frame(seq_path, f_id) for f_id in frame_ids]   ## 取RGB图片
        seq_event_path = self._get_event_img_sequence_path(seq_id)
        frame_event_img_list = [self._get_frame(seq_event_path, f_id) for f_id in frame_ids]  ## 取dvs事件图片
        ...
        # frame_event_list = self._get_event_frame(seq_event_path, frame_ids)   ## 取事件

        return frame_event_img_list, anno_frames, object_meta
        # return frame_list, anno_frames, object_meta, frame_event_list, frame_event_img_list     # frame_event_img_list
